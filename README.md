...
| 自动清理日志                   | `wget https://noodb.com/sharkshell/chk_logs/auto_clean_log.sh` | 自动清理指定目录下超过5天的日志文件，当目录大小超过1GB时触发清理 | */5 * * * * /opt/shark-shell/chk_logs/auto_clean_log.sh |
| 自动清理thingsboard ts_kv 分表 | `wget https://noodb.com/sharkshell/chk_logs/pg_clean.sh` | 自动清理thingsboard ts_kv 分表；保留最近三个月 | */5 * * * * /opt/shark-shell/chk_logs/pg_clean.sh |
| 防火墙初始化 | `wget https://noodb.com/sharkshell/firewall/firewall_init.sh` | 初始化iptables防火墙规则，设置默认策略和允许端口 | 无 |
| IP白名单管理 | `wget https://noodb.com/sharkshell/firewall/white_table.sh` | 管理IP白名单和端口访问控制 | 无 |
...
| 软件                 | 下载链接                                                         | 安装命令                                            |
| -------------------- | ---------------------------------------------------------------- | --------------------------------------------------- |
| openeuler 安装docker | `wget https://noodb.com/sharkshell/dockers/docker_install_el.sh` | `bash docker_install_el.sh`                         |
| 通用linux安装docker  | `wget https://noodb.com/sharkshell/dockers/docker_install.sh` | `bash docker_install.sh`                         |
